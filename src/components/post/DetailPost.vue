      ...
      <v-row>
          ...
          <v-col class="px-3" style="position: relative;" md="4">
            ...
            >{{ post.user.fullname }} <v-spacer></v-spacer>
                <!-- <v-btn color="transparent" v-if="post.user.id != currentUser"
            ...
            <v-card-title class="pa-0" style="font-size: 17px">{{ post.description }}</v-card-title>
            ...
            <div class="bg-scroll-cmt" style="overflow-y: scroll">
                <div 
                class="d-flex mb-2" 
                v-for="comment in post.comments" 
                :key="comment.id">
                    <div>
                        <v-img style="border-radius: 100%;" width="50px" height="50px" :src="'https://docs.google.com/uc?id=' + comment.user.avatar"></v-img>
                    </div>
                    <div class="ml-2">
                        <div class="pa-0">{{ comment.user.fullname }}</div>
                        <div class="pa-0" style="font-size: 14px;">{{ comment.content }}</div>
                    </div>
                </div>
            </div>
            <div style="position: absolute; bottom:0;right:12px;left:0;">
                <v-textarea 
                rows="2" 
                class="textarea-cmt" 
                color="white"
...
export default {
  mixins: [postMixin],
  props: ["post", "followings", "listPost"],
  data(){
      return{
          currentUser: null,
          txtComment: null
